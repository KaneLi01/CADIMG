...
def load_inference_models(args):
    ...
    trained_cn_path = os.path.join(args.parent_cn_path, args.index, "ckpt/controlnet_epoch25.pth")
    cn_checkpoint = torch.load(trained_cn_path, map_location=args.device)

    if 'controlnet' in cn_checkpoint:
        cn_state_dict = cn_checkpoint['controlnet']
    else:
        cn_state_dict = cn_checkpoint  # 如果本身就是纯 state_dict
        
    controlnet.load_state_dict(cn_state_dict)
...
if __name__ == "__main__":
    ...
    output_path = f'/home/lkh/siga/output/temp/722/result/{args.index}/{input_name}.png'  # 输出图像路径
