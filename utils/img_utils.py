...
def find_black_white_edges(image):
    """
    检测PIL图像中黑色和白色像素的交界处
    
    Args:
        image: PIL.Image对象，应该是黑白图像
        
    Returns:
        PIL.Image对象，边界像素为黑色，其他部分为白色
    """
    # 转换为灰度图像（如果不是的话）
    if image.mode != 'L':
    ...
    # 遍历每个像素
    for y in range(1, height - 1):  # 避免边界
        for x in range(1, width - 1):
            ...
            for dy, dx in directions:
                ...
                neighbor_pixel = binary[neighbor_y, neighbor_x]
                
                # 如果当前像素与邻居像素颜色不同，说明是边界
...
def stack_imgs(img1, img2, mode='b', output_path=None):
    """
    将图1叠加在图2上。根据黑色部分叠加，mode=b时只叠加黑色部分；mode=eb时只叠加非黑色部分
    """
    ...
    if mode == 'b':
        mask = np.all(img2 == [0, 0, 0], axis=-1)  # shape: (H, W)，bool数组
    elif mode == 'eb':
